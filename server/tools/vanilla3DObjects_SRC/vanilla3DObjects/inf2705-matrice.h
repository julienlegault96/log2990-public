...
public:
   ...
   void Translate(glm::vec3 move) {
	   matr_.top() = glm::translate(matr_.top(), move);
   }
   void Rotate( GLfloat angle, GLfloat x, GLfloat y, GLfloat z )
   { matr_.top() = glm::rotate( matr_.top(), (GLfloat)glm::radians(angle), glm::vec3(x,y,z) ); }
   void Rotate(GLfloat rotation, glm::vec3 axe)
   {
	   matr_.top() = glm::rotate(matr_.top(), (GLfloat)glm::radians(rotation), glm::vec3(axe.x, axe.y, axe.z));
   }
